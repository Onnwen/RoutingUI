...
public class HelloController {
    ...
    @FXML AnchorPane graphView = new AnchorPane();
    @FXML AnchorPane routeGraphView = new AnchorPane();
    ...
    SmartGraphPanel<String, String> graph = new SmartGraphPanel<>(g, strategy);
    ...
    public void initialize() {
        ...
        graphView.getChildren().setAll(new AnchorPane(new SmartGraphContainer(graph)));

        networksListView.getSelectionModel().select(0);
        updateSelectedNetwork(0);
    ...
    public void updateGraph() {
        ...
        for(Device device:selectedNetwork.getDevices()) {
            g.insertVertex(device.getName());
        }

        for(Route route:selectedNetwork.getRoutes()) {
            g.insertEdge(route.getOne().getName(), route.getTwo().getName(), route.getId().toString());
        }
    ...
    public void updateRouteGraph(int index) {
        new Thread( new Runnable() {
            public void run()  {
                try  { Thread.sleep( 500 ); }
                catch (InterruptedException ie)  {}
                graph.getStylableVertex(startingDevice.getValue().toString()).setStyleClass("selected");
                graph.getStylableVertex(endingDevice.getValue().toString()).setStyleClass("selected");
                for(Route route: routesFound.getSortedRoutes()[index].getRoutes()) {
                    graph.getStylableEdge(route.getId().toString()).setStyleClass("selectedEdge");
                }
                for(Device device: routesFound.getSortedRoutes()[index].getDevices()) {
                    graph.getStylableEdge(device.getName()).setStyleClass("selectedVertex");
                }
            }
        } ).start();
    }
    ...
    public void updateStatus() {
        for(Vertex vertex: g.vertices()) {
            graph.getStylableVertex(vertex).setStyleClass("vertex");
        }

        if (startingDevice.getSelectionModel().getSelectedIndex() != -1) {
            graph.getStylableVertex(startingDevice.getValue().toString()).setStyleClass("selected");
        }
        if (endingDevice.getSelectionModel().getSelectedIndex() != -1) {
            graph.getStylableVertex(endingDevice.getValue().toString()).setStyleClass("selected");
        }
